...
可以自己编译，进入src文件夹里打开terminal make 就能够编译成静态链接库DGHVlib.a 该同态加密库是在ubuntu环境下完成的，用到了linux系统日期函数，需要在linux环境下使用。

将生成的DGHVlib.a和dghv.h头文件复制到test文件夹中，可以运行测试代码。

--------crypto--------
run  :   make
run  :   gcc test.o dghvlib.a  -o test.exe -lgmp -lm
run  :   ./test.exe

--------api--------
make
gcc api.o apilib.a -pthread -o api.exe -lm
./api.exe

--------together-------
make
gcc api.o test.o apilib.a dghvlib.a -pthread -o api.exe -lgmp -lm
./api.exe
