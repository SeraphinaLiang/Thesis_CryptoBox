...
int initial(){
    ...
    read_sec_para(para, "../key_setting/parameters");
    // read key
    init_sk(&prikey, para);
    init_pkset(&pubkey, para);

    printf("read_pubkey\n");
    read_pubkey(pubkey, "../key_setting/public_key.key");
    printf("read_prikey\n");
    read_prikey(prikey, "../key_setting/private_key.key");
...
int reset(int level){
    ...
    save_sec_para(para, "../key_setting/parameters");
    // generate key
    init_sk(&prikey, para);
    init_pkset(&pubkey, para);
    ...
    // save key
    printf("save_pubkey\n");
    save_pubkey(pubkey, "../key_setting/public_key.key");
    printf("save_prikey\n");
    save_prikey(prikey, "../key_setting/private_key.key");
...
char decrypt(){
    ...
    FILE *f = fopen("../decrypt_text/cin.txt","r");
    gmp_fscanf(f,"%Zd",&cin);
   // gmp_printf("%Zd\n",cin);
...
int generatePKmod(){
   ...
   // gmp_printf("pk0= %Zd\n", pubkey->pks[0]);
    ...
    FILE *out = fopen("../key_setting/pk0.txt", "wt");
    gmp_fprintf(out, "%Zd\n",pubkey->pks[0]);
    fclose(out);
